<?php

namespace App\Http\Controllers\store;

use App\Http\Controllers\Controller;
use App\Models\Addon;
use App\Models\Edit;
use App\Models\Product;
use App\Models\ProductAddon;
use App\Models\ProductCategory;
use App\Models\ProductEdit;
use App\Models\ProductSauce;
use App\Models\ProductSize;
use App\Models\Sauce;
use App\Models\Store;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Storage;

class ProductsController extends Controller
{
    public function index(Store $store)
    {
        get_locale();
        $products = $store->products();
        return view('products.index', ['products' => $products]);
    }
    public function add()
    {
        get_locale();
        $store_id = get_current_store();
        $store = Store::with(['currency', 'product_categories', 'addons', 'edits', 'sauces', 'sizes'])->find($store_id);
        //dd($store);
        $cates = $store->product_categories;
        $currency = $store->currency;
        return view('store.products.add', [
            'cates' => $cates,
            'store' => $store,
            'addons' => $store->addons,
            'edits' => $store->edits,
            'sauces' => $store->sauces,
            'lang_code' => $store->lang_code,
            'currency' => $currency,
            'sizes' => $store->sizes
        ]);
    }
    public function edit($id)
    {
        $store_id = get_current_store();
        $store = Store::with(['product_categories', 'currency', 'addons', 'edits', 'sauces', 'sizes'])->find($store_id);
        $cates = $store->product_categories;
        $product = Product::with(['product_addons', 'product_edits', 'product_sauces'])->find($id);
        $currency = $store->currency;
        return view('store.products.update', [
            'cates' => $cates,
            'addons' => $store->addons,
            'edits' => $store->edits,
            'sauces' => $store->sauces,
            'product' => $product,
            'lang_code' => $store->lang_code,
            'currency' => $currency,
            'sizes' => $store->sizes
        ]);
    }
    /**
     * create new product
     * @param \Illuminate\Http\Request $request
     * @return \Illuminate\Http\Response
     */
    public function create(Request $request)
    {
        $store = Store::find(get_current_store());
        request()->validate([
            'name_ar' => 'nullable|max:50',
            'name_en' => 'nullable|max:50',
            'price' => 'required',
            'product_category_id' => 'required',
            'is_active' => 'required',
            'is_recommended' => 'nullable',
            'description_ar' => 'nullable|max:255',
            'description_en' => 'nullable|max:255',
            'cooking_time' => 'required',
            'image' => 'required',
            'addons' => 'nullable|array',
            'edits' => 'nullable|array',
            'sauces' => 'nullable|array',
            'sizes' => 'nullable|array',
            'discount' => 'nullable',
            'discount_type' => 'nullable',
            'tax' => 'nullable',
            'tax_type' => 'nullable'
        ]);
        $errors = array();
        if (check_lang_code($store->lang_code) != null) {
            switch ($store->lang_code) {
                case 'ar':
                    if (empty($request['name_ar'])) $errors['name_ar'] = __('messages.please_enter_name_ar');
                    if (empty($request['description_ar'])) $errors['description_ar'] = __('messages.please_enter_description_ar');
                    break;
                case 'en':
                    if (empty($request['name_en'])) $errors['name_en'] = __('messages.please_enter_name_en');
                    if (empty($request['description_en'])) $errors['description_en'] = __('messages.please_enter_description_en');

                    break;
                default:
                    if (empty($request['name_ar']) || empty($request['name_en'])) $errors['name'] = __('messages.please_enter_name_both');
                    if (empty($request['description_ar']) || empty($request['description_en'])) $errors = __('messages.please_enter_description_both');
            }
        }
        if (!empty($errors)) return back()->withErrors($errors);
        $data = [
            'price' => $request['price'],
            'product_category_id' => $request['product_category_id'],
            'is_active' => $request['is_active'],
            'cooking_time' => $request['cooking_time'],
            'store_id' => $store->id,
            'image' => $request['image']
        ];
        if (isset($request['is_recommended'])) $data['is_recommended'] = $request['is_recommended'];
        if (isset($request['tax'])) $data['tax'] = $request['tax'];
        if (isset($request['tax_type'])) $data['tax_type'] = $request['tax_type'];
        if (!empty($request['name_ar'])) $data['name_ar'] = $request['name_ar'];
        if (!empty($request['name_en'])) $data['name_en'] = $request['name_en'];
        if (!empty($request['description_ar'])) $data['description_ar'] = $request['description_ar'];
        if (!empty($request['description_en'])) $data['description_en'] = $request['description_en'];
        // if (isset($request['size'])){
        //     $sizes = '';
        //     foreach($request['size'] as $size)
        //         $sizes .= $size.',';
        //     $data['size'] = $sizes;
        // }
        if (isset($request['discount']) && isset($request['discount_type'])) {
            $data['discount'] = $request['discount'];
            $data['discount_type'] = $request['discount_type'];
        }
        $product = Product::create($data);
        if ($product) {
            if (isset($request['addons'])) {
                $data = array();
                foreach ($request['addons'] as $addon) {
                    array_push($data, [
                        'store_id' => $store->id,
                        'product_id' => $product->id,
                        'addon_id' => $addon
                    ]);
                }
                ProductAddon::insert($data);
            }
            if (isset($request['edits'])) {
                $data = array();
                foreach ($request['edits'] as $edit) {
                    array_push($data, [
                        'store_id' => $store->id,
                        'product_id' => $product->id,
                        'edit_id' => $edit
                    ]);
                }
                ProductEdit::insert($data);
            }
            if (isset($request['sauces'])) {
                $data = array();
                foreach ($request['sauces'] as $sauce) {
                    array_push($data, [
                        'store_id' => $store->id,
                        'product_id' => $product->id,
                        'sauce_id' => $sauce
                    ]);
                }
                ProductSauce::insert($data);
            }
            if (isset($request['sizes'])) {
                $data = array();
                foreach ($request['sizes'] as $size) {
                    array_push($data, [
                        'store_id' => $store->id,
                        'product_id' => $product->id,
                        'size_id' => $size
                    ]);
                }
                ProductSize::insert($data);
            }
            return back()->with(['success' => __('messages.create_success')]);
        }
    }
    public function update(Request $request, $id)
    {
        get_locale();
        $product = Product::find($id);
        $store = Store::find(get_current_store());
        if ($product->store_id != $store->id) return back()->withErrors(['errors' => __("messages.update_permission_error")]);
        request()->validate([
            'price' => 'required',
            'product_category_id' => 'required',
            'is_active' => 'required',
            'cooking_time' => 'required',
            'image' => 'nullable',
            'addons' => 'nullable|array',
            'edits' => 'nullable|array',
            'sauces' => 'nullable|array',
            'sizes' => 'nullable|array',
            'discount' => 'nullable',
            'discount_type' => 'nullable',
        ]);
        $errors = array();
        if (check_lang_code($store->lang_code) != null) {
            if (empty($request['description_' . $store->lang_code])) $errors['descriprion'] = __('messages.please_enter_description_' . $store->lang_code);
            if (empty($request['name_' . $store->lang_code])) $errors['name'] = __('messages.please_enter_name_' . $store->lang_code);
        } else {
            if (empty($request['description_en']) || empty($request['description_ar']))
                $errors['description'] = __('messages.please_enter_description_both');
            if (empty($request['name_ar']) || empty($request['name_en']))
                $errors['name'] = __('messages.please_enter_name_both');
        }
        if (!empty($errors)) return back()->withErrors($errors);
        $data = [
            'price' => $request['price'],
            'product_category_id' => $request['product_category_id'],
            'is_active' => $request['is_active'],
            'is_recommended' => 0,
            'cooking_time' => $request['cooking_time'],
            'store_id' => $store->id,
        ];
        if (isset($request['tax'])) $data['tax'] = $request['tax'];
        if (isset($request['tax_type'])) $data['tax_type'] = $request['tax_type'];
        if (!empty($request['name_ar'])) $data['name_ar'] = $request['name_ar'];
        if (!empty($request['name_en'])) $data['name_en'] = $request['name_en'];
        if (!empty($request['description_ar'])) $data['description_ar'] = $request['description_ar'];
        if (!empty($request['description_en'])) $data['description_en'] = $request['description_en'];
        // if (isset($request['size'])){
        //     $sizes = '';
        //     foreach($request['size'] as $size)
        //         $sizes .= $size.',';
        //     $data['size'] = $sizes;
        // }
        if (isset($request['discount']) && isset($request['discount_type'])) {
            $data['discount'] = $request['discount'];
            $data['discount_type'] = $request['discount_type'];
        }
        if (isset($request['image'])) $data['image'] = $request['image'];
        $product->update($data);
        if (isset($request['addons'])) {
            $data = array();
            ProductAddon::where('product_id', $product->id)->delete();
            foreach ($request['addons'] as $addon) {
                array_push($data, [
                    'store_id' => $store->id,
                    'product_id' => $product->id,
                    'addon_id' => $addon
                ]);
            }
            ProductAddon::insert($data);
        }
        if (isset($request['edits'])) {
            ProductEdit::where('product_id', $product->id)->delete();
            $data = array();
            foreach ($request['edits'] as $edit) {
                array_push($data, [
                    'store_id' => $store->id,
                    'product_id' => $product->id,
                    'edit_id' => $edit
                ]);
            }
            ProductEdit::insert($data);
        }
        if (isset($request['sauces'])) {
            ProductSauce::where('product_id', $product->id)->delete();
            $data = array();
            foreach ($request['sauces'] as $sauce) {
                array_push($data, [
                    'store_id' => $store->id,
                    'product_id' => $product->id,
                    'sauce_id' => $sauce
                ]);
            }
            ProductSauce::insert($data);
        }
        if (isset($request['sizes'])) {
            ProductSize::where('product_id', $product->id)->delete();
            $data = array();
            foreach ($request['sizes'] as $size) {
                array_push($data, [
                    'store_id' => $store->id,
                    'product_id' => $product->id,
                    'size_id' => $size
                ]);
            }
            ProductSize::insert($data);
        }
        return back()->with(['success' => __('messages.update_success')]);
    }
    public function delete($id)
    {
        $store_id = get_current_store();
        $product = Product::find($id);
        if ($product == null) return abort(404);
        if ($product->store_id != $store_id) return abort(404);
        ProductSauce::where('product_id', $product->id)->delete();
        ProductEdit::where('product_id', $product->id)->delete();
        ProductAddon::where('product_id', $product->id)->delete();
        ProductSize::where('product_id', $product->id)->delete();
        Storage::disk('public')->delete($product->image);
        $product->delete();
        return back()->with(['success' => __('messages.delete_success')]);
    }
    public function uploadPhoto(Request $request)
    {
        if (isset($request['image'])) {
            $data = $request['image'];
            $image_array_1 = explode(";", $data);

            //base64,

            $image_array_2 = explode(",", $image_array_1[1]);

            

            $data = base64_decode($image_array_2[1]);

            $image_name = '/stores/products/' . time() . '.png';
            Storage::disk('public')->put($image_name, $data);
            //file_put_contents($image_name, $data);
            return $image_name;
        }
    }
}
