    ...
    <div class="container col-md-11 shadow-lg">
        <form method="POST" action="{{route('admin.admins.create')}}">
            ...
            <div class="mb-3">
                ...
                <select name="site_privilege_id" class="form-select">
                    @foreach($privileges as $privilege)
                    <option value="{{$privilege->id}}" @if($privilege->id == old('site_privilege_id')){{'selected'}}@endif>{{App::isLocale('ar')?$privilege->name_ar:$privilege->name_en}}</option>
                    @endforeach
                </select>
            </div>
