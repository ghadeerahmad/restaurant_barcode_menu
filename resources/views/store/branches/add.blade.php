...
<section id="main">
    <div class="container col-md-11 shadow-lg">
        <form action="{{url('store/branches/add')}}" method="POST">
            @csrf
            @if($errors->any())
            ...
            <div class="row">
                <div class="col-md-2">
                    ...
                    <input type="hidden" id="logo" name="logo" value="{{old('image')}}">
            ...
            <div class="input-group mb-3">
                <input type="text" class="form-control" name="delivery_fees" value="{{old('delivery_fees')}}" placeholder="{{__('branches.delivery_fees')}}" aria-label="{{__('branches.delivery_fees')}}" aria-describedby="basic-addon1">
                <span class="input-group-text" id="basic-addon1">{{$store->currency->code}}</span>
            </div>

            <div class="mb-3">
                <label class="form-label">{{__('branches.delivery_area')}} <span id="area">{{old('delivery_distance')??0}}</span> km</label>
                <input type="range" class="form-range" name="delivery_distance" value="{{old('delivery_distance')??0}}" onchange="addCircle(this.value)">
            </div>
            @endif
            @endforeach
            <div class="mb-3">
                <input type="hidden" id="lat" name="latitude" value="{{old('latitude')}}">
                <input type="hidden" id="lng" name="longtude" value="{{old('longtude')}}">
