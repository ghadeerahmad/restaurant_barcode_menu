...
@section('title',__('chef.update_product'))
@section('css')
...
<div id="main">
    <div class="container shadow-lg">
        <form action="{{url('store/products/update',$product->id)}}" method="POST" role="form">
            @csrf
            @if($errors->any())
            ...
            <div class="row">
                <div class="col-md-2">
                    ...
                    <input type="hidden" id="logo" name="image" value="{{$product->image}}">
                    <div class="image_area">
                        
                            <label for="upload_image">
                                <img src="@if($product->image != null) {{ asset('storage'.$product->image) }} @else {{asset('storage/stores/shops.png')}} @endif" id="uploaded_image" class="img-responsive img-circle" width="100%" />
                ...
                <input type="number" class="form-control" name="price" placeholder="{{__('chef.price')}}" value="{{$product->price}}">
            ...
            <div class="mb-3">
                ...
                <select class="selectpicker form-control" name="product_category_id" data-live-search="true" style="height: 200px">
                    ...
                    <option value="{{ $cat->id }}"  {{$product->product_category_id == $cat->id?'selected':''}}>{{ App::isLocale('ar')?$cat->name_ar:$cat->name_en }}</option>
                    @endforeach
                </select>
            </div>
            <div class="mb-3">
                ...
                <select class="form-select" name="is_active">
                    <option value="1" {{$product->is_active == 1?'selected':''}}>{{__('chef.yes')}}</option>
                    <option value="0" {{$product->is_active == 0?'selected':''}}>{{__('chef.no')}}</option>
                </select>
            </div>
                ...
                <!-- <div class="col-md-3">
                    <div class="mb-3">
                        ...
                        <select class="selectpicker form-control" name="is_recommended">
                            ...
                            <option value="0" {{$product->is_recommended == 0?'selected':''}}>{{ __('chef.no') }}</option>
                            <option value="1" {{$product->is_recommended == 1?'selected':''}}>{{ __('chef.yes') }}</option>
                        </select>
                    </div>
                </div> -->
                <div class="col-md-4">
                    <div class="mb-3">
                        ...
                        <select class="selectpicker form-control" name="addons[]" multiple="multiple" data-live-search="true" style="height: 200px">
                            ...
                            <option value="{{ $addon->id }}">{{ App::isLocale('ar')?$addon->name_ar:$addon->name_en }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                <div class="col-md-4">
                    <div class="mb-3">
                        ...
                        <select class="selectpicker form-control" name="edits[]" multiple="multiple" data-live-search="true" style="height: 200px">
                            ...
                            <option value="{{ $edit->id }}">{{ App::isLocale('ar')?$edit->name_ar:$edit->name_en }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                <div class="col-md-4">
                    <div class="mb-3">
                        ...
                        <select class="selectpicker form-control" name="sauces[]" multiple="multiple" data-live-search="true" style="height: 200px">
                            ...
                            <option value="{{ $sauce->id }}">{{ App::isLocale('ar')?$sauce->name_ar:$sauce->name_en }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                ...
                <div class="col-md-6">
                    <div class="mb-3">
                        ...
                        <select class="selectpicker form-control" name="discount_type">
                            <option value="AMOUNT" {{$product->discount_type == 'AMOUNT'?'selected':''}}>{{__('chef.amount')}}</option>
                            <option value="PERCENT" {{$product->discount_type == 'PERCENT'?'selected':''}}>{{__('chef.percent')}}</option>
                        </select>
            ...
            <div class="row">
                <div class="col-md-6">
                    <div class="mb-3">
                        ...
                        <input type="number" class="form-control" name="cooking_time" value="{{$product->cooking_time}}">
